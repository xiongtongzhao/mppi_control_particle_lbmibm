...
#        return self.F_true(state, u, t) if self.step_dependency else self.F_true(state, u) # added by lailai
    ...
    def command(self, state_targets):
        """
        :param state: (nx) or (K x nx) current state, or samples of states (for propagating a distribution of states)
        :returns action: (nu) best action
        """
        state = state_targets[0]
        self.reach_targets = state_targets[1]
        #print(state,'command')
...
#            self.cost_total += self.running_cost3(state[:,:self.nps],state[:,self.nps:], self.reach_targets)
            ...
            '''
            self.cost_total +=self.running_cost(state, u, prev_state,d_state, self.reach_targets)
            ## d_state added by lailai
...
def run_mppi(mppi,ENV,rank,rrank,comm, retrain_dynamics, retrain_after_iter=50, iter=1000, ifzero=False, render=True):
    ...
    if rank==rrank:
        ...
        env=None
    if rank ==rrank:
        dataset = torch.zeros((retrain_after_iter, 2 + mppi.nu), dtype=mppi.U.dtype, device=mppi.d)
        total_reward = 0
        trajectory = []
        done = False
        reach_targets = None
...
#        print(i)
    ...
    #            print(env.state)
            ...
            if reach_targets is None:
                reach_targets = np.zeros(env.ntargets)

           # print(reach_targets)
            #print(reach_targets,'r')
            action = mppi.command([state, reach_targets])
...
#        print(action.detach().numpy())
        ...
        obs, r, done, _ = env.step(action.detach().numpy()) #obs: np array
        if rank ==rrank:         
            reach_targets = obs[mppi.nx:] #n_tars
            result_step = np.concatenate((obs[:mppi.nx], action.detach().numpy(), np.array(r), int(done)), axis = None)
            #print(s)
            if ( i==0 ):
                trajectory = result_step
            else:
                trajectory = np.vstack( (trajectory, result_step) )
            torch.save(trajectory,'traj.pt')
            total_reward += r
        ...
        ##the dataset is cleaned and collect data from true dynamics every 'retrain_after_iter' steps.
            ...
            dataset[di, :2] = torch.tensor(state[:2], dtype=mppi.U.dtype).reshape(dataset[di, :2].shape)            
            dataset[di, 2:] = action
    if rank ==rrank:            
    #env.close()
    #mppi.reset()
        ...
        niter_actual = i + 1
    if rank ==rrank:     
        return total_reward, dataset, trajectory, done, niter_actual, reach_targets
    else:
