...
#    noise_sigma = torch.tensor([[ACTION_SIG,0,0,0,0,0,0],[0,ACTION_SIG,0,0,0,0,0],[0,0,ACTION_SIG,0,0,0,0],[0,0,0,ACTION_SIG,0,0,0], [0,0,0,0, ACTION_SIG,0,0],[0,0,0,0,0, ACTION_SIG,0],[0,0,0,0,0,0, ACTION_SIG]], device=d, dtype=dtype) ## shape nu * n
    ...
    def running_cost(state,action, prev_state,dstate,reach_targets):
        ...
        prev_p=prev_state.view(1,-1).repeat(position.shape[0],1)
        #print(prev_p.shape)
        #print(reach_targets,'r')
...
#        cost = WEIGHT_POSITION * torch.sum(((position - current_tar) ** 2) , dim=1)  + WEIGHT_ACTION * torch.sum(action ** 2, dim=1)  + WEIGHT_ANGLE * cost_the
        ...
        dist=(position[:,:2] - position[:,2:4]).norm(dim=1)+(position[:,:2] - position[:,4:6]).norm(dim=1)+(position[:,2:4] - position[:,4:6]).norm(dim=1) 
        dist_pre=(prev_p[:,:2] - prev_p[:,2:4]).norm(dim=1)+(prev_p[:,:2] - prev_p[:,4:6]).norm(dim=1)+(prev_p[:,2:4] - prev_p[:,4:6]).norm(dim=1) 
        #print(dist_pre[0])
        cost = WEIGHT_POSITION * torch.sum(((position - current_tar)**2 ) , dim=1) / prev_dist_sqr + WEIGHT_ACTION * torch.sum(action ** 2, dim=1) + WEIGHT_ANGLE * cost_the+WEIGHT_DIST*dist/dist_pre
...
#        print(e)
        ...
        X1=0.15
        Y1=-0.5+0.15/math.sqrt(3)
        theta=0
        R=0.3/math.sqrt(3)
        X_tar3 = [ X1 + R*math.cos(theta)]
        Y_tar3 = [ Y1 + R*math.sin(theta)]
        X_tar2 = [X1 + R*math.cos(theta+2*math.pi/3.0)]
        Y_tar2 = [ Y1 + R*math.sin(theta+2*math.pi/3.0)]
        X_tar = [ X1 + R*math.cos(theta+4*math.pi/3.0)]
        Y_tar = [Y1 + R*math.sin(theta+4*math.pi/3.0)]   
        POSITION_INI = torch.Tensor([X_ini, Y_ini,X_ini2, Y_ini2,X_ini3, Y_ini3]).t()
        POSITION_TAR = torch.Tensor([X_tar, Y_tar,X_tar2, Y_tar2,X_tar3, Y_tar3]).t()
        ...
        env.x_tar = np.array([X_tar, Y_tar,X_tar2, Y_tar2,X_tar3, Y_tar3], dtype=np.float32) ## 2 rows * ntargets columns
        env.dist_epsi_goal = DIST_EPSI_GOAL
        env.ACTION_LOW = ACTION_LOW
        env.ACTION_HIGH = ACTION_HIGH
        env.state = np.array([X_ini, Y_ini,X_ini2, Y_ini2,X_ini3, Y_ini3])      
        #print(env.reach_targets)
        if rank==root_rank:
        ...
        if rank==root_rank:        
            ...
            ini_tar = np.array(([N_tar] + [X_ini] + X_tar + [Y_ini] + Y_tar+[X_ini2] + X_tar2 + [Y_ini2] + Y_tar2 + [X_ini3] + X_tar3 + [Y_ini3] + Y_tar3))
