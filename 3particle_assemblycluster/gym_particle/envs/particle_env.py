...
#        reward_action = COEF_EXCEED*reward_exceed(q, self.max_rate, -self.max_rate)
        ...
        #print(np.linalg.norm(self.state_n[:2]-x_tar[:2])+ np.linalg.norm(self.state_n[2:4]-x_tar[2:4])+ np.linalg.norm(self.state_n[4:6]-x_tar[4:6]))        
        if np.linalg.norm(self.state_n[:2]-self.state_n[2:4])<0.2 or np.linalg.norm(self.state_n[:2]-self.state_n[4:6])<0.2 or np.linalg.norm(self.state_n[2:4]-self.state_n[4:6])<0.2:
            done = True
            print(done)
        ...
        if min(d_min_in1,d_min_in2) < DIST_EPSI_INLET: #exit the inlet
            ...
            reward += 0
        '''  
        if np.linalg.norm(self.state_n[:2])>0.9  or np.linalg.norm(self.state_n[2:4]) >0.9 or np.linalg.norm(self.state_n[4:6]) >0.9:
            done = True
            return self._get_obs(), reward, done, {}             

        #print (self.state_n)
        #print (x_tar)
        print(abs(np.linalg.norm(self.state_n[:2]-self.state_n[2:4])-0.3),abs(np.linalg.norm(self.state_n[4:6]-self.state_n[2:4])-0.3),abs(np.linalg.norm(self.state_n[4:6]-self.state_n[:2])-0.3))

        if abs(np.linalg.norm(self.state_n[:2]-self.state_n[2:4])-0.3)<0.012 and abs(np.linalg.norm(self.state_n[4:6]-self.state_n[2:4])-0.3)<0.012 and abs(np.linalg.norm(self.state_n[4:6]-self.state_n[:2])-0.3)<0.012:
    ...
    def dynamics(self,state, perturbed_action):
        q = self.tanh_limit(perturbed_action, ACTION_LOW, ACTION_HIGH)
        #q = torch.clamp(perturbed_action, ACTION_LOW, ACTION_HIGH)
        if state.dim() is 1 or q.dim() is 1:
            state = state.view(1, -1) # data of one row
            q = q.view(1, -1)
        statexy=state
        q_n = self.transform(q,ACTION_LOW,ACTION_HIGH)
        for i in range(5):
            xu = torch.cat((statexy, q_n), dim=1) ## cat along the 1 dimension (0: along column direction; 1: along row direction)
            #print(network(xu).detach().view(1,-1))
            #print(self.rever_transform(network(xu).detach(),MINVX,MAXVX),MINVX,MAXVX)
            U=network(xu).detach()
            U=U.view(1,-1)            
            U[:,0]=self.rever_transform(U[:,0],MINVX,MAXVX)
            U[:,1]=self.rever_transform(U[:,1],MINVY,MAXVY)           
            U[:,2]=self.rever_transform(U[:,2],MINVX,MAXVX)
            U[:,3]=self.rever_transform(U[:,3],MINVY,MAXVY)
            U[:,4]=self.rever_transform(U[:,4],MINVX,MAXVX)
            U[:,5]=self.rever_transform(U[:,5],MINVY,MAXVY)            
            k1 =0.01*U
    ...
    def transform(self,input_,min_value,max_value):
        input_=2.0*(input_-min_value)/(max_value-min_value)-1.0
        return input_
    
    def rever_transform(self,input_,min_value,max_value):
        #print(input_)
        input_=(0.5*(input_+1.0)*(max_value-min_value)+min_value)
        return input_        
        
    def tanh_limit(self,action, act_l, act_h):
        act_m = (act_l + act_h)/2
    ...
    def reset(self):
        self.it=0
        self.state = np.array([X_ini, Y_ini,X_ini2, Y_ini2,X_ini3, Y_ini3],dtype=np.float32)
        self.reach_targets = np.zeros(self.ntargets, dtype=int)        
